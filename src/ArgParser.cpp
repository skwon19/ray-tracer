...
ArgParser::ArgParser(int argc, const char *argv[]) 
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        // rendering options
            ...
            shadows = true;
        } else if (!strcmp(argv[i], "-indirectBounces")) {
            i++; assert (i < argc); 
            indirectBounces = atoi(argv[i]);
        } else if (!strcmp(argv[i], "-subsamples")) {
            i++; assert (i < argc); 
            subsamples = atoi(argv[i]);
        }
    ...
    std::cout << "- indirectBounces: " << indirectBounces << std::endl;
    std::cout << "- subsamples: " << subsamples << std::endl;
}
