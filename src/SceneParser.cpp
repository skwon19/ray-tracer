...
SceneParser::parseLights() 
{
    ...
    while (_num_lights > count) {
        ...
        {
            AreaLight * parsedLight = parseAreaLight();
            // lights.push_back(parseAreaLight());
            // areaLights.push_back(parseAreaLight());
            lights.push_back(parsedLight);
            areaLights.push_back(parsedLight);
        }
        else {
...
AreaLight *
SceneParser::parseAreaLight() 
{ // TODO: implement
    char token[MAX_PARSER_TOKEN_LENGTH];
    Vector3f p1, p2, p3, color;
    float flux = 0;
    getToken(token); assert(!strcmp(token, "{"));
    while (true) {
        getToken(token); 
        if (!strcmp(token, "p1")) {
            ...
            p2 = readVector3f();
        } else if (!strcmp(token, "p3")) {
            p3 = readVector3f();
        } else if (!strcmp(token, "color")) {
            color = readVector3f();
        } else if (!strcmp(token, "flux")) {
            flux = readFloat();
        } else {
            assert(!strcmp(token, "}"));
            break;
    ...
    return new AreaLight(p1, p2, p3, color, flux);
}
...
SceneParser::parseMaterial() 
{
    ...
    while (true) {
        ...
        else if (strcmp(token, "reflectance")==0) {
            reflectance = readFloat();
        } 
        else if (strcmp(token, "glossiness")==0) {
            glossiness = readFloat();
        } 
        else if (strcmp(token, "transmittance")==0) {
            transmittance = readInt();
        } 
    ...
    Material *answer = new Material(diffuseColor, specularColor, shininess, reflectance, glossiness, transmittance);
